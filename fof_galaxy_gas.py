...
#cellThreshold == The minimum size of the disjoint set
#returns[i] : Indices of all the particles that lie in one cloud/cluster
def getDisjointSets_gas(*, particlePositions, particleDensities,densityCut, linkingLength, cellThreshold = 10):
...
#Find all the points whose distance from each other is at most r
    ...
    for i in range(len(clusters)):
        ...
        if(visited[i] == 0):
            ...
            if(len(clustertoAppend) > cellThreshold): 
                disjointSets.append(indices[clustertoAppend]); 
...
# (clouds_tags, cloud_p_ids) = getDisjointSets(particlePositions=particlePositions, particleDensities = particle_density, tags = tag,densityCut=densityCutOff, linkingLength = linkingLength, cellThreshold = 100);

(clouds_tags) = getDisjointSets_gas(particlePositions=particlePositions, particleDensities = particle_density,densityCut=densityCutOff, linkingLength = linkingLength, cellThreshold = 100);
...
#plt.colorbar(im);
plt.savefig('fof_clouds_gas_{}.png'.format(plt_number));