...
def make_clumps(*,ds, threshold_clump_density, saveName):
	...
	stepsize = 2;
	find_clumps(master_clump,threshold_clump_density, threshold_clump_density*(1.2), 2);
...
parser.add_argument('-ovd', type = float, nargs = 1, help  = 'overdensity parameter');  #overdensity parameter
...
clumpName = "clump_threshold_density_{}_{}_{}.h5".format(args.ovd[0], args.name[0], args.plt_number[0]);
...
threshold_density_cm = return_density(args.ovd[0]);
print("Making the clump file with the fileName {} \n Threshold_number_density = {}".format(clumpName, args.ovd[0]));

make_clumps(ds = ds, threshold_clump_density = threshold_density_cm, saveName = out_dir+clumpName);
